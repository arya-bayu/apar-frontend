...
export default function CategoryPage() {
    ...
    useEffect(() => {
        if (!category || redirected) return;

        const correctSlug = formatNameForSlug(category?.data?.name);

        if (slug !== correctSlug) {
            setRedirected(true);
            router.replace(`/store/${id}/${correctSlug}`);
        }
    ...
    const title = `Jual ` + category?.data?.name + ` | ` + process.env.NEXT_PUBLIC_APP_NAME + ` - Marketplace Alat Pemadam Kebakaran No. 1 di Bali`
        ...
        <div>
            ...
            <main>
                <section className="flex border-b justify-center bg-monza-50 bg-gradient-to-b from-transparent via-transparent to-stone-50">
                    <div className="max-w-7xl w-full flex flex-col lg:flex-row items-center px-6 lg:px-12 pt-12 space-y-12 lg:space-y-0 lg:space-x-0">
                        <div className={cn(
                            (category?.data?.image && category?.data?.features.length > 0) ? 'lg:w-1/3 lg:mr-6' : 'lg:w-2/3',
                            `lg:pb-12`
                        )}>
                        ...
                        <div className={cn(
                            (category?.data?.image && category?.data?.features.length > 0) ? 'lg:w-2/3' : 'lg:w-1/3',
                            `flex flex-col-reverse lg:flex-row gap-12 lg:gap-6`
                        )}>
                            {category?.data?.image && (
                                <div className={`${category?.data?.features.length > 0 && 'lg:w-1/2'} flex justify-center items-end`}>
                                    <Image src={process.env.NEXT_PUBLIC_BACKEND_URL + `/storage/` + category?.data?.image?.path} height={1000} width={1000} alt={"Foto " + category?.data?.name} className="object-contain object-bottom h-[25rem]" priority />
                                </div>
                            )}

                            {category?.data?.features.length > 0 && (
                                <div className={cn(
                                        ...
                                        return (
                                            <div className="group gap-4 flex flex-col md:flex-row items-start justify-start md:items-center lg:p-6">
                                                <div className="border border-zinc-200 bg-zinc-50 group-hover:bg-zinc-50/80 group-hover:drop-shadow-md rounded-lg p-3">
                                                    {React.createElement((Lucide as any)[feature.icon ?? 'Plus'], { size: 30, className: "text-monza-500" })}
                                                </div>
