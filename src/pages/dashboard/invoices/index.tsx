...
import { id as idLocale, is } from 'date-fns/locale';
...
const Invoices = () => {
  ...
  const [exportIds, setExportIds] = useState<IInvoice['id'][] | undefined>(undefined);
  ...
  const handleExportData = async (
    ...
    setExportIds(id)
    setExportDialogDescription(`Filter ${id?.length ?? invoices?.data?.totalRowCount} data yang akan di export`)
          ...
          {can('create invoices') && (
            <Link href="invoices/new">
              <Button size="sm" className={`uppercase ${isBelowSm ? 'px-2' : ''}`}>
                {isBelowSm ? <PlusIcon size={18} /> : 'Buat Invoice'}
              </Button>
            </Link>
          )}
      ...
      >
        <div className="flex flex-col gap-4 py-4">
          ...
          <div className="flex flex-col justify-start gap-1">
            ...
            <CustomerCombobox
              hasReset={selectedCustomerId ? true : false}
          ...
          <div className="flex items-center">
            ...
            >
              Kelompokkan produk berdasarkan No. Invoice
            </label>
          </div>
        </div>
      </CustomExportDialog>
